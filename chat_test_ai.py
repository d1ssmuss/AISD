...
В массивах возможны ходов, не должны быть ходы, где есть фигуры
    ...
    for j in range(y-1, y+2): # y
        for i in range(x-1, x+2): # x
            if (0 <= i <= 7 and 0 <= j <= 7) and (i != x or y != j):
                possible_moves_king.append([i,j])
    return f"{possible_moves_king}, Расположение Короля {x} {y}, len:{len(possible_moves_king)}"
    # вроде работает
    ...
    for j in range(0, 8): # y
        for i in range(0, 8): # x
            if (0 <= i <= 7 and 0 <= j <= 7) and (i != x or y != j) and (x == i and j != y or x != i and j == y):
                possible_moves_rook.append([i,j])
    return f"{possible_moves_rook}, Расположение Ладьи {x} {y}, len:{len(possible_moves_rook)}"
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:  # 
            ...
            if selected_piece:
                ...
                if selected_piece == 'white_king' and (abs(row - white_king_pos[0]) <= 1 and abs(column - white_king_pos[1]) <= 1):
                    ...
                    # Отрисовка клеток с кружочками по-середине
                    
                    # тест
                    pygame.draw.circle(screen, (0, 0, 255), (500, 500), 20)
